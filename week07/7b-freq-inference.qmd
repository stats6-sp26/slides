...
```{r}
#| echo: false
#| fig-alt: A histogram showing the distribution of base_pay on the x-axis and count on the y-axis. The data is multi-modal, with a significant spike near zero and a primary cluster of values centered around 100,000. The distribution is right-skewed, with base pay extending up to 300,000, though counts drop significantly after 150,000.
lapd |>  
...
## Population parameter vs. point estimate of the parameter
...
| Description                              | Parameter of Interest | Point Estimate / Sample Statistic |
|-------------------------------|-----------------------|-----------------------------------|
| Mean                          | $\mu$                 | $\bar x$                          |
...
#| cache: true
#| fig-align: center
#| fig-alt: Histogram that looks symmetric and bell-shaped centered around about 85,000. The x axis reads Sample Means and y axis reads count. In the center of the x axis the the count in the y-axis reaches close to 1000. The sample means below 80,000 and above 90,000 do not even reach counts of 100.
...
#| cache: true
#| fig-align: center
#| fig-alt: Same plot as the previous histogram with a vertical line passing through about 85000 on the x axis.
...
$$(p_1 - p_2) \sim \text{approximately } N((\pi_1 - \pi_2),$$
